...
def train_model(net, loss_type, learning_rate, trainloader, validloader, device, data_type, epochs=1000, gamma = 0.001,
    ...
    for epoch in range(epochs): 
        for i, data in enumerate(trainloader, 0):
            ...
            outputs = net(inputs)
            loss_mse,loss_rrmse,loss_shape,loss_temporal = torch.tensor(0),torch.tensor(0),torch.tensor(0),torch.tensor(0)
            ...
            history_losses_training['training'].append(loss.item())
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if(verbose):
            if (epoch % print_every == 0):
                ...
                print('Eval mse= ', final_mse ,
                      ...
                      ' ramp= ', final_ramp)
                history_losses_eval['mse'].append(final_mse)
                history_losses_eval['dtw'].append(final_dtw)
                history_losses_eval['tdi'].append(final_tdi)
                history_losses_eval['hausdorff'].append(final_hausdorff)
                history_losses_eval['ramp'].append(final_ramp)
                if wandb_logger:
                    logger.log({
                        'eval_mse': final_mse,
                        'eval_dtw': final_dtw,
    ...
    if wandb_logger:
        wandb.finish()
    return history_losses_training, history_losses_eval
...
def compare_models(training, net_gru_dilate, net_gru_mse, net_gru_dtw,net_gru_rrmse, trainloader, 
...
def compare_gammas(gammas, output_length, device, batch_size, trainloader, 
    ...
    for gamma in gammas:
        ...
        history_losses_training, history_losses_eval = train_model(
            net=net_gru_dtw,
            loss_type='dilate',
            learning_rate=0.001,
            trainloader=trainloader,
        ...
        print('mse= ', dtw_mse,
              ...
              ' hausdorff= ', dtw_hausdorff,
              ' ramp= ', dtw_ramp) 
        print("During training info:######################")

        print("Training : ", history_losses_training)

        print("Eval : ", history_losses_eval)
